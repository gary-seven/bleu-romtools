...
{
    return editbuf[cposx + (cposy * Drivers[DriverNumber]->RomData->MapData[mapno].width)];
}
...
void MapCursor(BITMAP * playfield, int x, int y)
{
    ...
    gl = Drivers[DriverNumber]->GfxInfo[GetGfxBank()].GfxLayout;
    rd = Drivers[DriverNumber]->RomData;
	...
	*/
	...
	case (','): // palette --
	    if (ModifyPaletteNumber(-1))
	    	SetColours(Drivers[DriverNumber]->GfxInfo[CurrentBank].GfxLayout,0);
	    break;
...
void EditMap(void)
{
    ...
    if (Drivers[DriverNumber]->RomData == NULL)
    {
	Dialog_Box("Sorry. No ROM data available", "Hit any key to continue");
	return;
    }
    if (Drivers[DriverNumber]->RomData->MapData == NULL)
    {
    ...
    if (!LoadRomData())
    {
	struct GFXLAYOUT * gl = Drivers[DriverNumber]->GfxInfo[GetGfxBank()].GfxLayout;
	if (TempCharL)
	...
	maped_current = Drivers[DriverNumber]->RomData->MapData[mapno].funcs;
    ...
    //sprintf(txt, "Map at 0x%04lx", Drivers[DriverNumber]->RomData->MapData[0].StartAddress);
    //Dialog_Box(txt,txt);
    if (dataroms) {
	free(dataroms);
	dataroms = NULL;
