...
//  see additional document: pmdata.txt
//
...
void Maped_Playfield_HC_PacMan(BITMAP * playfield)
{
    ...
    for (count = 0 ;
         count < Drivers[DriverNumber]->RomData->MapData[mapno].numHardcoded ;
	 count ++)
	{
	    x= Drivers[DriverNumber]->RomData->MapData[mapno].hardcoded[2*count];
	    y= Drivers[DriverNumber]->RomData->MapData[mapno].hardcoded[(2*count)+1];
...
void Maped_Info_PacMan(int mode)
{
    ...
    max = Drivers[DriverNumber]->RomData->MapData[mapno].OverlayCount;
...
struct MAPED_FUNCTIONS MsPacMan_Functions =
{
    // this is not correct, but it will hold the place for now...
    NULL, NULL, NULL, NULL,
    NULL, NULL, NULL
};
